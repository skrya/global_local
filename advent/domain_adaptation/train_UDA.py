...
from advent.model.discriminator import get_fc_pix_discriminator_1x1
...
def train_advent(model, trainloader, targetloader, cfg):
    ...
    device = torch.device(cfg.CUDA_devices) 
    ...
    # feature-level
    d_aux = get_fc_pix_discriminator_1x1(num_classes=num_classes)
    d_aux.train()
    

    # seg maps, i.e. output, level
    d_main = get_fc_pix_discriminator_1x1(num_classes=num_classes)
    d_main.train()
    ...
    if cfg.GPU_ID < 0 :
        ...
        d_aux = nn.DataParallel(d_aux)
        d_main = nn.DataParallel(d_main)
    model.to(device)
    d_aux.to(device)
    d_main.to(device)
    ...
    for i_iter in tqdm(range(cfg.TRAIN.EARLY_STOP)):
        ...
        pred_src_aux, pred_src_main = model(images_source.to(device))
        ...
        if viz_tensorboard:
            log_losses_tensorboard(writer, current_losses, i_iter*cfg.TRAIN.BATCH_SIZE_SOURCE)

            if i_iter % cfg.TRAIN.TENSORBOARD_VIZRATE == cfg.TRAIN.TENSORBOARD_VIZRATE - 1:
                draw_in_tensorboard(writer, images, i_iter*cfg.TRAIN.BATCH_SIZE_SOURCE, pred_trg_main, num_classes, 'T')
                draw_in_tensorboard(writer, images_source, i_iter*cfg.TRAIN.BATCH_SIZE_SOURCE, pred_src_main, num_classes, 'S')
