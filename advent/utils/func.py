...
def mse_loss_trans(y_pred, y_truth_tensor):
    y_truth_tensor = y_truth_tensor.unsqueeze(1).to(y_pred.get_device())
    return nn.MSELoss()(y_pred, y_truth_tensor)
...
def bce_loss_trans(y_pred, y_truth_tensor):
    y_truth_tensor = y_truth_tensor.unsqueeze(1).to(y_pred.get_device())
    return nn.BCEWithLogitsLoss()(y_pred, y_truth_tensor)

'''def bce_loss_pix(y_pred, y_label):
    y_truth_tensor = torch.FloatTensor(y_pred.size())
    y_truth_tensor.fill_(y_label)
    y_truth_tensor = y_truth_tensor.to(y_pred.get_device())
    return nn.BCEWithLogitsLoss()(y_pred, y_truth_tensor)'''
